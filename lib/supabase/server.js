...
const createSupabaseServerClient = () => {
  ...
  const createStubServerClient = () => {
    ...
    const stubCountResponse = { data: null, error: null, count: 0 };
    ...
    const queryMethods = () => {
      const methods = {
        ...
        select: (columns, options) => {
          // If this is a count query, return count response
          if (options && options.count) {
            return Object.assign(Promise.resolve(stubCountResponse), queryMethods());
          }
          return queryMethods();
        },
      };
      ...
      return Object.assign(Promise.resolve(stubResponse), methods);
    };
