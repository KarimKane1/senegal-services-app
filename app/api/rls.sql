...
alter table if exists connection_request enable row level security;

-- Users can read requests they sent or received
create policy cr_select_participant on connection_request
  for select using (
    requester_user_id = auth.uid() or recipient_user_id = auth.uid()
  );

-- Only authenticated users can insert and must be the requester
create policy cr_insert_requester on connection_request
  for insert to authenticated with check (requester_user_id = auth.uid());

-- Only the recipient can update (approve/deny) or the requester can cancel
create policy cr_update_participant on connection_request
  for update using (
    recipient_user_id = auth.uid() or requester_user_id = auth.uid()
  );
...
create policy connection_select_participant on connection
  for select using (
    user_a_id = auth.uid() or user_b_id = auth.uid()
  );

-- Only authenticated users can insert connections
create policy connection_insert_auth on connection
